...
with {
  ...
  elucidate = emacs: emacs.override {
    withGTK2 = false;
    withGTK3 = false;
  };
};
{
  overrides = {
    emacs = elucidate (trace "FIXME: Emacs from 19.03 has a broken display"
                             self.nixpkgs1809).emacs;
  };

  tests =
    with {
      checkDisplay = emacs: label: self.runCommand "test-emacs-${label}"
        { buildInputs = [ self.fail emacs self.xvfb-run-safe ]; }
        ''
    ...
    };
    {
      ...
      warbo-tests = self.runCommand "emacs-test-runner"
        {
          config = self.patchShebang {
            dir = self.latestGit {
              url = http://chriswarbo.net/git/warbo-emacs-d.git;
            };
          };
          __noChroot  = true;
          buildInputs = [
