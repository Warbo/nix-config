...
with rec {
  ...
  get =
    with builtins;
    path: x: if path == [ ] then x else get (tail path) (getAttr (head path) x);

  image = get attrPath configured;

  ubootable =
    nix-helpers.nixpkgs.runCommand "${image.name}-ubootable"
      { inherit image uboot; }
      ''
        ...
        do
          ...
          # We need to overwrite some of the image with UBoot, so the microSD
          # can boot on a stock Pinephone's UBoot (i.e. without having TowBoot).
          ...
          #  - 8: boots the card's UBoot, but stalls at Stage1 due to a
...
if uboot == null then image else ubootable
